...
SpellCastResult Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    bool const spellForcesMoveInterrupt = (m_spellInfo->Id == 770001);
    if ((m_spellInfo->IsChanneled() || m_casttime) && m_caster->IsPlayer() && m_caster->isMoving() && ((m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_MOVEMENT) || spellForcesMoveInterrupt) && !IsTriggered())
    {
...
void Spell::update(uint32 difftime)
{
    ...
    if ((m_caster->IsPlayer() && m_timer != 0) &&
            m_caster->isMoving() && ((m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_MOVEMENT) || spellForcesMoveInterrupt) && m_spellState == SPELL_STATE_PREPARING &&
...
SpellCastResult Spell::CheckItems()
{
    ...
    else
    {
        ...
        // Custom Spell Mastery spell: Volcanic Eruption (770001) is a ground-target spell and should not require equipped weapon class.
        if (m_spellInfo->Id != 770001 && m_caster->IsPlayer() && !m_caster->ToPlayer()->HasItemFitToSpellRequirements(m_spellInfo))
            return SPELL_FAILED_EQUIPPED_ITEM_CLASS;
    }
